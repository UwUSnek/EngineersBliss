...
vec2 _internal_calculate_lensed_uv(sampler2D SceneDepthSampler, vec3 centerWorld, vec2 screenUV, vec2 uvToModify, float horizon, float outerRadius, float spin){
    ...
    // Pull the fragment's screen UV back into the undistorted local space.
    ...
    //float sigma = maxHorizonDistance * 0.5;
    //float lensStrength = exp(-pow((distance - horizon) / sigma, 2.0));
...
//  * @param spin Frame dragging strength/direction. -1 to +1. 0 means no drag.
//  **/
// vec2 _internal_calculate_lensed_uv(sampler2D SceneDepthSampler, vec3 axisX, vec3 axisY, vec3 centerWorld, vec2 screenUV, vec2 uvToModify, float horizon, float outerRadius, float spin){
...
//     mat4 M = getProjMatrix() * getViewMatrix();
//     vec4 clip0 = M * vec4(centerWorld, 1.0);

//     vec2 screenCenter = clip0.xy / clip0.w * 0.5 + 0.5;

//     // vec2 screenCenter = worldToScreenUV(centerWorld);

//     // float eps = 1.0;
//     // vec2 basisX = (worldToScreenUV(centerWorld + axisX * eps) - screenCenter) / eps;
//     // vec2 basisY = (worldToScreenUV(centerWorld + axisY * eps) - screenCenter) / eps;
//     // mat2 localToScreen = mat2(basisX, basisY);
//     vec4 dclipX = M * vec4(axisX, 0.0);
//     vec4 dclipY = M * vec4(axisY, 0.0);
//     vec2 basisX = (dclipX.xy * clip0.w - clip0.xy * dclipX.w) / (clip0.w * clip0.w) * 0.5;
//     vec2 basisY = (dclipY.xy * clip0.w - clip0.xy * dclipY.w) / (clip0.w * clip0.w) * 0.5;
//     mat2 localToScreen = mat2(basisX, basisY);
...
// vec2 calculate_lensed_custom_uv(sampler2D SceneDepthSampler, vec3 axisX, vec3 axisY, vec3 centerWorld, vec2 uvToModify, float horizon, float outerRadius, float spin){
//     vec2 screenUV = gl_FragCoord.xy / vec2(textureSize(SceneDepthSampler, 0));
//     return _internal_calculate_lensed_uv(SceneDepthSampler, axisX, axisY, centerWorld, screenUV, uvToModify, horizon, outerRadius, spin);
// }
// vec2 calculate_lensed_screen_uv(sampler2D SceneDepthSampler, vec3 axisX, vec3 axisY, vec3 centerWorld, float horizon, float outerRadius, float spin){
//     vec2 screenUV = gl_FragCoord.xy / vec2(textureSize(SceneDepthSampler, 0));
//     return _internal_calculate_lensed_uv(SceneDepthSampler, axisX, axisY, centerWorld, screenUV, screenUV, horizon, outerRadius, spin);
// }