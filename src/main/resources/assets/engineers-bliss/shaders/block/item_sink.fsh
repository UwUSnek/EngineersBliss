...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    //vec2 uv0 = (fragCoord / iResolution.xy) * (iResolution.xy / min(iResolution.x, iResolution.y));

    //vec2 uv0 = (fragCoord - 0.5 * iResolution.xy) / min(iResolution.x, iResolution.y) + 0.5;
    //if(max(abs(uv0.x), abs(uv0.y)) > 1.0) uv0 = vec2(-999.0, -999.0);

    vec2 uv0 = fragCoord / iResolution.xy;
    vec2 squareUV = (fragCoord - 0.5 * iResolution.xy) / min(iResolution.x, iResolution.y) + 0.5;
    if(max(abs(squareUV.x - 0.5), abs(squareUV.y - 0.5)) > 0.5) uv0 = vec2(-999.0, -999.0);
    ...
    vec2 diskUV;
        vec3 _planePosLocal = frame.rayOrigin + frame.rayDir * rayLen; //TODO remove
    if(hitPlane) {
        vec3 planePosLocal = frame.rayOrigin + frame.rayDir * rayLen;
        diskUV = planeUV(planePosLocal, diskTangent, diskBitangent, 1.0);
    }
    // Lensing now takes the lensing object's WORLD-SPACE center (frame.center) rather
    // than a flat-plane UV. frame.center is already correct in both the MINECRAFT path
    // (real world position) and the shader-toy path (world origin), so this one function
    // call covers both #ifdef branches identically.
    #define LENSING_SCALE 2.0
    // vec2 lensedScreenUV = calculate_lensed_screen_uv(sceneDepthSampler, frame.center, horizon, horizon * LENSING_SCALE, 0.0);
    // vec2 lensedScreenUV = calculate_lensed_screen_uv(
    vec2 lensedScreenUV = calculate_lensed_screen_uv(
        sceneDepthSampler, frame.center,
        // normalize(frame.uAxis), normalize(frame.vAxis),
        horizon, horizon * LENSING_SCALE, 0.0
    );
    ...
    // float tNear, tFar;
    // bool hitCore = intersectSphere(frame.rayOrigin, frame.rayDir, horizon, tNear, tFar) && tFar >= 0.0;
    ...
    //fragColor = vec4(length(uv) < horizon);
    //fragColor = vec4(hitCore);
    //fragColor = vec4(uv + vec2(0.5), 0.0, 1.0);
    //fragColor = vec4(lensedUV + vec2(0.5), 0.0, 1.0);

    #ifndef MINECRAFT
        if(uv0.x < -1.0) fragColor = vec4(uv0, 0.0, 1.0);
    #endif
    //fragColor = vec4(pow(abs(diskUV), vec2(11)), 0.0, 1.0);
    //fragColor = vec4(abs(_planePosLocal), 1.0);
    //fragColor = vec4(rayLen);
    //fragColor = over(vec4(0.0, 0.0, 0.0, length(uv) < 1.0 && length(uv) > horizon), fragColor);
}
...
//     #define LENSING_SCALE 2.0
//     // vec2 lensedScreenUV = calculate_lensed_screen_uv(sceneDepthSampler, frame.center, horizon, horizon * LENSING_SCALE, 0.0);
//     // vec3 axisX = normalize(frame.uAxis);
// // vec3 axisY = normalize(frame.vAxis);
// // axisY = normalize(frame.vAxis - axisX * dot(axisY, axisX));


// mat4 camToWorld = inverse(getViewMatrix());
// vec3 axisX = normalize(camToWorld[0].xyz);
// vec3 axisY = normalize(camToWorld[1].xyz);
//     vec2 lensedScreenUV = calculate_lensed_screen_uv(
//         sceneDepthSampler, frame.center,
//         // normalize(frame.uAxis), normalize(frame.vAxis),
//         axisX, axisY,
...
// // fragColor = vec4(normalize(frame.uAxis) * 0.5 + 0.5, 1.0);
// }
