...
//     sampler2D sceneColorSampler = sceneSampler;
//     sampler2D sceneDepthSampler = sceneDepthSampler;
//     float t = GameTime * 1200.0;
// #else
//     sampler2D sceneColorSampler = iChannel0;
//     sampler2D sceneDepthSampler = iChannel1;
//     float t = iTime;
// #endif
...
//     vec2 uv = fragCoord / iResolution.xy - vec2(0.5) * iResolution.x / iResolution.y;
// #endif
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(max(abs(uv0.x), abs(uv0.y)) > 1.0) uv0 = vec2(0.0, 0.0);
    ...
    float horizon = 0.6; // world scale radius
    float outerRadius = horizon * 2.0;
    ...
    vec2 lensedUV = calculate_lensed_uv(sceneDepthSampler, diskUV, horizon, outerRadius, 0.35);
    //vec2 lensedUV = calculate_lensed_uv(sceneDepthSampler, uv, horizon, 0.35);
    ...
    vec4 bgColor = over(objectBgColor, texture(sceneColorSampler, lensedUV));
    ...
    //fragColor = vec4(lensedUV, 0.0, 1.0);
}