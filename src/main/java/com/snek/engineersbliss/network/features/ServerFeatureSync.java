...
import com.snek.engineersbliss.network.creative_tweaks.payloads.BoolFeatureUpdateRequestPayload;
import com.snek.engineersbliss.network.creative_tweaks.payloads.FloatFeatureUpdateRequestPayload;
import com.snek.engineersbliss.network.creative_tweaks.payloads.IntFeatureUpdateRequestPayload;
 ...
 * It keeps track of the current state of all features of any connected player.
 */
public class ServerFeatureSync {
    private ServerFeatureSync() {}
    private static final Map<UUID, ServerFeaturePlayerData> playerData = new ConcurrentHashMap<>();




    private static ServerFeaturePlayerData getPlayerData(final Player player) {
        return playerData.compute(player.getUUID(), (k, v) -> v == null ? new ServerFeaturePlayerData() : v);
    }
    ...
    public static <T> void setFeature(final Player player, final __base_ServerFeature<T> feature, final T value) {
     ...
     * //TODO do something abt this^ ? idk how its gonna work
     */
    public static <T> boolean serverPlayerHasFeature(final Object entity, final __base_ServerFeature<T> feature, final T value) {
     ...
     * //TODO do something abt this^ ? idk how its gonna work
     */
    public static <T> boolean serverPlayerHasFeature(final Object entity, final __base_ServerFeature<T> feature) {
        if(feature instanceof ServerToggleFeature) {
            return serverPlayerHasFeature(entity, (__base_ServerFeature<Boolean>)feature, true);
        }
        return false;
    }
    public static boolean shouldPlayerPhaseThroughBlocks(final Object entity) {
        return serverPlayerHasFeature(entity, CreativeTweakServerFeatureSet.PHASE_THROUGH_BLOCKS_FLY) && ((Player)entity).getAbilities().flying;
    }
